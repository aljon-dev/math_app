...
class _DefinitionSectionState extends State<ParabolaDefinitionSection> {
  ...
  Future<void> play() async {
    try {
      if (currentStep == 0) {
        await player.setAsset('assets/Audio/parabola_step1.mp3');
        ...
        await player.setAsset('assets/Audio/Parabola_step2.mp3');
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Playing', style: TextStyle(color: Colors.white)), backgroundColor: Colors.green));
        await player.play();
      } else if (currentStep == 2) {
        await player.setAsset('assets/Audio/Parabola_step3.mp3');
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Playing', style: TextStyle(color: Colors.white)), backgroundColor: Colors.green));
        await player.play();
      }
    } catch (e) {
    ...
    return Center(
      child: SizedBox(
        ...
        child: GestureDetector(
          ...
          child: Hero(tag: 'image-$imagePath', child: Image.asset(imagePath, width: double.infinity, height: 200, fit: BoxFit.contain, errorBuilder: (context, error, stackTrace) => Center(child: Text('Image not found', style: TextStyle(color: Colors.grey))))),
        ),
      ),
    );
  }
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            IconButton(onPressed: () => _scaffoldKey.currentState?.openEndDrawer(), icon: Icon(Icons.menu)),
          ],
        ),
        endDrawer: Drawer(
          child: ListView(
        ...
        body: Column(
          children: [
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (currentStepData['image'] != null) ...[_buildZoomableImage(context, currentStepData['image']), const SizedBox(height: 20)],
            ...
            Container(padding: const EdgeInsets.all(16.0), decoration: BoxDecoration(color: Colors.grey[100], boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.3), spreadRadius: 1, blurRadius: 3, offset: const Offset(0, -2))]), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [ElevatedButton.icon(onPressed: currentStep > 0 ? previousStep : null, icon: const Icon(Icons.arrow_back), label: const Text('Back'), style: ElevatedButton.styleFrom(backgroundColor: currentStep > 0 ? Colors.grey[300] : Colors.grey[200], foregroundColor: currentStep > 0 ? Colors.black : Colors.grey, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12))), Row(children: List.generate(steps.length, (index) => Container(margin: const EdgeInsets.symmetric(horizontal: 3), width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: index == currentStep ? Colors.blue : Colors.grey[300])))), ElevatedButton.icon(onPressed: currentStep < steps.length - 1 ? nextStep : null, icon: const Icon(Icons.arrow_forward), label: Text(currentStep < steps.length - 1 ? 'Next' : 'Done'), style: ElevatedButton.styleFrom(backgroundColor: currentStep < steps.length - 1 ? Colors.blue : Colors.green, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12)))])),
          ],
        ),
      ),
    );
