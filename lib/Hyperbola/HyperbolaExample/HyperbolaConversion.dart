...
class HyperbolaConversionsScreen extends StatelessWidget {
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              _buildConversionCard(title: '1. Transform General form to standard equation', problem: 'Write the standard form of Hyperbola: 9x² - 2y² + 54x + 4y - 11 = 0', steps: ['1st: Group all the x\'s and all y\'s but start with positive x² or y²', '(9x² + 54x) - (2y² - 4y) = 11', '2nd: Factor out the coefficient of x² and y² then simplify', '9(x² + 6x) - 2(y² - 2y) = 11', '3rd: Completing the square', 'For x: (6/2)² = 9 | For y: (2/2)² = 1', '9(x² + 6x + 9) - 2(y² - 2y + 1) = 11 + 81 - 2', '4th: Factoring and simplify', '9(x + 3)² - 2(y - 1)² = 90', '5th: Equation must be equal to 1', '(x + 3)²/10 - (y - 1)²/45 = 1'], context: context, answer: 'Final Answer: (x + 3)²/10 - (y - 1)²/45 = 1', graphImage: 'assets/images/hyperbola_graph1.jpg'),
              const SizedBox(height: 16),
              _buildConversionCard(title: '2. Transform Standard form to General Form', problem: 'Transform: (y - 8)²/25 - (x - 4)²/16 = 1', steps: ['1st: Multiply by LCD (25)(16)', '16(y - 8)² - 25(x - 4)² = 400', '2nd: Expand the squared terms', '16(y² - 16y + 64) - 25(x² - 8x + 16) = 400', '3rd: Distribute', '16y² - 256y + 1024 - 25x² + 200x - 400 = 400', '4th: Arrange to General Form', '-25x² + 16y² + 200x - 256y + 224 = 0'], context: context, answer: 'Final Answer: -25x² + 16y² + 200x - 256y + 224 = 0', graphImage: 'assets/images/hyperbola_graph2.jpg'),
            ],
          ),
        ),
      ),
  ...
  Widget _buildConversionCard({required String title, required String problem, required List<String> steps, required String answer, required BuildContext context, String? graphImage}) {
    return Card(
      elevation: 4,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(problem, style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black)),
            const SizedBox(height: 12),
            ...steps.map((step) => Padding(padding: const EdgeInsets.symmetric(vertical: 4.0), child: Text(step, style: TextStyle(color: Colors.black)))).toList(),
            const SizedBox(height: 12),
            Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.blue[50], borderRadius: BorderRadius.circular(8)), child: Text(answer, style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black))),
            if (graphImage != null) ...[const SizedBox(height: 12), _buildZoomableImage(graphImage, context)],
          ],
        ),
      ),
    );
  ...
  Widget _buildZoomableImage(String imagePath, BuildContext context) {
    return SizedBox(
      ...
      child: GestureDetector(
        ...
        child: Hero(tag: 'image-$imagePath', child: ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.asset(imagePath, width: double.infinity, height: 200, fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => Container(height: 200, decoration: BoxDecoration(color: Colors.black, borderRadius: BorderRadius.circular(8)), child: Center(child: Text('Image not found: $imagePath', style: TextStyle(color: Colors.black))))))),
      ),
    );
  }
}
