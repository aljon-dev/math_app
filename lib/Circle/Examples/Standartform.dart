...
class StandardFormScreen extends StatelessWidget {
  ...
  Widget _buildDerivationCard(BuildContext context) {
    return Card(elevation: 3, margin: const EdgeInsets.only(bottom: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(padding: const EdgeInsets.all(16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Deriving the Standard Form', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold, color: Colors.orange[800])), const SizedBox(height: 12), const Text('Using the distance formula with points (h,k), (x,y) and distance r:', style: TextStyle(fontSize: 16)), const SizedBox(height: 8), _buildMathFormula(r'd = \sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2}'), const SizedBox(height: 8), const Text('Substitute values:'), _buildMathFormula(r'r = \sqrt{(x - h)^2 + (y - k)^2}'), const SizedBox(height: 8), const Text('Square both sides:'), _buildMathFormula(r'r^2 = (x - h)^2 + (y - k)^2'), _buildMathFormula(r'(x - h)^2 + (y - k)^2 = r^2'), const SizedBox(height: 8), const Text('For center at (0,0):'), _buildMathFormula(r'r^2 = x^2 + y^2')])));
  }
