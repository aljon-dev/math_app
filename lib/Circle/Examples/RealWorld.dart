...
class RealWorldProblemsScreen extends StatelessWidget {
  ...
  Widget _buildProblemCard(BuildContext context, String title, String problem, String solution, String imagePath) {
    return Card(elevation: 3, margin: const EdgeInsets.only(bottom: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(padding: const EdgeInsets.all(16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold, color: Colors.brown[800])), const SizedBox(height: 12), Text(problem, style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontSize: 16, height: 1.5)), const SizedBox(height: 12), Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(8)), child: Text(solution, style: const TextStyle(fontSize: 15, fontFamily: 'RobotoMono', height: 1.5))), const SizedBox(height: 12), Center(child: Image.asset(imagePath, width: 200, height: 200, fit: BoxFit.contain))])));
  }
}
