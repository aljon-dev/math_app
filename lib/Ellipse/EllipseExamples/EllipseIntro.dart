...
class EllipseIntroScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopScope(child: Scaffold(appBar: AppBar(title: const Text('Real-life Ellipse Examples')), body: SingleChildScrollView(padding: const EdgeInsets.all(16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildExampleCard(context, 'Running Track Field', 'A running track field is an oval-shaped path designed for running, usually with two straight sides and two curved ends.', '1. The track is designed as an ellipse to allow smooth turns and maximize the running area.\n2. The curved ends help runners maintain speed and safety.\n3. The ellipse shape ensures fairness in lane distances.', 'assets/running_track.png'), const SizedBox(height: 20), _buildExampleCard(context, 'Football', 'A football is ellipse-shaped because of its importance in aerodynamics, better grip/handling, and unpredictable bounce which affects gameplay.', '1. The elliptical shape helps the ball travel farther and more accurately when thrown.\n2. It provides better grip and handling for players.\n3. The unpredictable bounce adds excitement to the game.', 'assets/football.jpg'), const SizedBox(height: 20), _buildExampleCard(context, 'Swimming Pool', 'Architects use elliptical shapes in designs for both aesthetic and functional reasons. Ellipses create visually pleasing and dynamic spaces.', '1. The ellipse shape allows for efficient use of space and water circulation.\n2. It provides a unique and attractive appearance.\n3. The design can accommodate more swimmers comfortably.', 'assets/swimming_pool.jpg')]))));
  }

  Widget _buildExampleCard(BuildContext context, String title, String problem, String solution, String imagePath) {
    return Card(elevation: 3, margin: const EdgeInsets.only(bottom: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(padding: const EdgeInsets.all(16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold, color: Colors.black)), const SizedBox(height: 12), Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(8)), child: Text(problem, style: const TextStyle(fontSize: 16, fontFamily: 'RobotoMono'))), const SizedBox(height: 12), Text('Reasons:', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text(solution, style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontSize: 16, height: 1.5)), const SizedBox(height: 12), Center(child: _buildZoomableImage(context, imagePath))])));
  }
